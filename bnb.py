...
def calcular_rota_otima(lojas, capacidade_caminhao):
    ...
    def backtrack_rota(rota_atual, lojas_restantes, cargas, combustivel_total):
        ...
        if len(cargas) > capacidade_caminhao: # se a capacidade foi excedida, desconnsidera a rota / criterio de poda
            return
        ...
        #faz uma heuristica para explorar primeiro as lojas mais promissoras em termos de menor consumo de combustível.
        lojas_restantes.sort(key=lambda loja: calcular_lower_bound(rota_atual, lojas_restantes, capacidade_caminhao))

        #branch and bound em si
